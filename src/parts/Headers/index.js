...
import { setModalLogin } from "../../states/Login/action";
import { setModalRegister } from "../../states/Register/action";
...
const Headers = () => {
  ...
  const ownUser = useSelector((state) => state.userData);
  return (
      ...
      <div className="mx-auto flex h-16 max-w-screen-xl items-center gap-8 px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-1 items-center justify-end md:justify-between">
          ...
          <div className="flex items-center gap-4">
            {ownUser?.name === undefined && (
              <div className="sm:flex sm:gap-4">
                <button
                  className="block rounded-md bg-cyan-500 px-5 py-2.5 text-sm font-medium text-white transition hover:bg-cyan-700 dark:hover:bg-cyan-500"
                  type="submit"
                  onClick={() => dispatch(setModalLogin(!modalLogin))}
                >
                  Login
                </button>

                <button
                  className="hidden rounded-md bg-gray-100 hover:bg-cyan-500 px-5 py-2.5 text-sm font-medium text-cyan-500 transition hover:text-white dark:bg-gray-800 dark:text-white dark:hover:text-white/75 sm:block"
                  type="submit"
                  onClick={() => dispatch(setModalRegister(!modalRegister))}
                >
                  Register
                </button>
              </div>
        ...
        {modalLogin && (
          <div className="fixed inset-0 z-10 overflow-y-auto shadow-[rgba(0,_0,_0,_0.24)_0px_3px_8px] ">
            <div
              ...
              onClick={() => dispatch(setModalLogin(!modalLogin))}
              onKeyPress={() => dispatch(setModalLogin(!modalLogin))}
              role="button"
              tabIndex="0"
            >
              <p>we</p>
                ...
                <div className="mt-3 sm:flex w-full">
                  <div className="mt-2 text-center sm:ml-1 sm:text-left w-full">
                    <div className="flex flex-col w-full">
                      ...
                      <div>
                        <Register modalRegister={modalRegister} />
                      </div>
                    </div>
                  </div>
                </div>
